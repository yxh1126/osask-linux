...
/* returns error code. just make it non-zero for now */
env is a 64KB work area. The first 8 dwords are used as return values.
Apparently volatile, seems unusable as instance variables. With JPEG_init
I think it is fine to create base_img only during decode.
%endif

[absolute 0]	;for nask, [section .bss] org 0 or win32 style
bmpinfo:
.regs:		resd 4
.reteip:	resd 1
.env:		resd 1
...
.endif:
	...
	;if size is small, trim height
	;should be readable to answer. For now, error
	push edx
	push eax
	...
	mov edx, eax
	add eax, byte 3			;assume size<1GB, zero edx
	...
	pop edx
	add esi, eax
	add esi, edx			;assume no padding at end of last line
...
;***************************************************************
; Made with small-size priority, but may be better than speed-priority ones.
; Only implementing lowest quality high-speed mode
...
.bpp24:
	...
	.do24.1:
	  ...
	  .do24.2:
	    ...
	    mov ax, [esi]
	     add esi, byte 3
	    ;grayscale processing: eax=24bitColor, edx=work, ecx=counter, ebx=work
	    ;could bring a routine killed off by Mr. Aikawa taste (lol
	    ;or bring a diffusion error routine
